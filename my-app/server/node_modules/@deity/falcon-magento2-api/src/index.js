...
module.exports = class Magento2Api extends Magento2ApiBase {
  ...
  async cart() {
    ...
    Logger.warn(`${this.name}: Loading cart again`);
    const emptyCart = {
      active: false,
      itemsQty: 0,
      items: [],
    ...
    if (!quoteId) {
      Logger.warn(`${this.name}: Loading cart again quoteId not have`);
      return emptyCart;
    }
  ...
  /**
   *
   * @returns {Promise<{items: {code: number, name: string}[]}>}
   */
  async loves(obj, { id }) {
    const response = await this.post(`/getcity`, {
      region: id
    }, { context: { useAdminToken: true } });
   ...
   */
  async snapToken(obj , { id }) {
    const response = await this.post('/snap/payment/redirect', { orderId: id });
    const { newQuoteId } = this.session;

    if (newQuoteId) {
      return { name: response.data, quoteId: newQuoteId };
      Logger.warn(`${this.name}: Snap token have new quote id`);
    }
    return { name: response.data, quoteId: '' };
  }

  /**
   * Get info Snap payment
   * @return {Promise<Address>} get info Snap data
   */
  async infoSnap(obj) {
    const response = await this.post('/snap/payment/info');

    const infoSnap = JSON.parse(response.data);

    return { is_production: infoSnap.is_production, client_id: infoSnap.client_id, is_active: infoSnap.active };
  }
   ...
   * @return {Promise<Address>} added address data
   */
  async vacodeSnap(obj , params) {
    const { va , oid } = params;
    const response = await this.post('/snap/payment/vacode', { va: va, oid: oid },  { context: { useAdminToken: true } });
  ...
  /**
   * post fail Snap payment
   * @return {Promise<Address>} post fail snap payment
   */
  async failSnap(obj, params) {
    const { order_id, quote_id } = params;
    const response = await this.post('/snap/payment/fail', { orderId: order_id, quoteId: quote_id },  { context: { useAdminToken: true } });

    const infoSnap = JSON.parse(response.data);
    Logger.warn(`${this.name}: ` + JSON.stringify(response));

    return { redirect: infoSnap.redirect, quote_id: infoSnap.quote_id };
  }
   ...
   * @returns {Promise<{items: {code: number, name: string}[]}>}
   */
  async kecamatans(obj, { id }) {
    const response = await this.post(`/getkecamatan`, {
      city: id
    }, { context: { useAdminToken: true } });
    Logger.debug(`${this.name}: ` + JSON.stringify(response));
    const kecamatans = response.data.map(item => ({
      id: item.id,
      name: item.kecamatan
    }));
    return { items: kecamatans };
  }
  ...
  async performCartAction(path, method, data) {
    Logger.debug(`${this.name}: ` + JSON.stringify(data));
    const { cart } = this.session;
  ...
  async placeOrder(obj, { input }) {
    let response;
    try {
      Logger.debug(`${this.name}: ` + JSON.stringify(input));
      response = await this.performCartAction('/order', 'put', Object.assign({}, input));
      Logger.debug(`${this.name}: ` + JSON.stringify(response));
      if (input.paymentMethod.method === 'snap') {
        this.session.afterOrder = true;
        const { cart, customerToken: { token } = {} } = this.session;
        Logger.warn(`${this.name}: create quote after place Order`);
        const cartPath = token ? '/carts/mine' : '/guest-carts';
        const newQuote = await this.post(cartPath);
        this.session.newQuoteId = newQuote.data;
      } else {
        this.session.afterOrder = false;
      }
    ...
    const orderId = parseInt(response.data.data, 10);
    Logger.warn(`${this.name}: get Last Order`);
    const getLastOrder = await this.post('/snap/payment/lastOrder', { orderId: orderId },  { context: { useAdminToken: true } });
    const newOrderId = getLastOrder.data;
    Logger.warn(`${this.name}: ` + newOrderId);
    const orderData = [ orderId, newOrderId];
    Logger.debug(`${this.name}: ` + JSON.stringify(orderData));
    ...
    var orderRealId = false;
    if (typeof orderData[0] != "undefined" && typeof orderData[1] != "undefined") {
      this.session.orderId = orderData[0];
      orderRealId = orderData[1];
    }

    if (!this.session.orderId || !orderRealId) {
      throw new Error('no order id from magento.');
    }

    this.session.orderQuoteId = this.session.cart.quoteId;
    var result = {"orderId" : this.session.orderId, "orderRealId": orderRealId};
    return result;
  }
   ...
   * @return {Promise<Address>} added address data
   */
  async lastOrderDetails(newOrderId) {
    Logger.warn(`${this.name}: get Last Order`);
    const response = await this.post('/snap/payment/lastOrder', { orderId: newOrderId },  { context: { useAdminToken: true } });
    const result = response.data;
    Logger.warn(`${this.name}: ` + result);
    return result;
  }
  ...
  removeCartData() {
    const { newQuoteId } = this.session;
    Logger.debug(`${this.name}: ` + JSON.stringify(Object.entries(this.session)));
    // delete this.session.cart;
    Logger.warn(`${this.name}: Delete cart session`);
    if (this.session.afterOrder === true) {
      Logger.warn(`${this.name}: remove Cart data ${newQuoteId}`);
      this.session.cart = { quoteId: newQuoteId };
      this.session.afterOrder = false;
    } else {
      delete this.session.cart;
    }
